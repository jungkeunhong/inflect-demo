...
import DetectAgentView from "./detect-agent-view"
...
// 테스트 단계 타입
type TestingPhase = 'analyst' | 'storyline' | 'detect' | 'general' | null
...
export default function AgentVisualization({
  ...
  // 감지 에이전트 판별 함수
  const isDetectAgent = (agent: Agent | null): boolean => {
    if (!agent) return false
    return agent.name.toLowerCase().includes('detect') || 
           agent.description?.toLowerCase().includes('detect') ||
           agent.tools.some((tool: string) => tool.toLowerCase().includes('anomaly') || tool.toLowerCase().includes('churn'))
  }
    ...
    const demoTimer = setTimeout(() => {
      if (activeAgent) {
        ...
        if (isAnalystAgent(activeAgent)) {
          ...
          setIsUserCommandReceived(true);
        } else if (isDetectAgent(activeAgent)) {
          setTestingPhase('detect');
          setIsUserCommandReceived(true);
        } else {
  ...
  const renderTestingPhaseContent = () => {
    ...
    if (isTesting && isUserCommandReceived) {
      switch (testingPhase) {
        ...
        case 'detect':
          return <DetectAgentView isActive={true} />;
        default:
    ...
    if (activeAgent) {
      if (isAnalystAgent(activeAgent)) {
        ...
        // We're now handling this in the BuilderPage
        return null;
      }
    }
