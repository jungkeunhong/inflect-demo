  ...
  const proceedToNextDemoStep = useCallback(() => {
    if (currentStep < demoConversationSteps.length && !isLoadingNextStep) {
      ...
      const isDuplicate = messages.some(msg => 
        msg.role === nextStepMessage.role && 
        msg.content === nextStepMessage.content
      );
      ...
      if (!isDuplicate) {
        ...
        // If duplicate, just advance the step without adding the message
        setCurrentStep(prev => prev + 1);
        setIsLoadingNextStep(false);
      }
    }
  }, [currentStep, addMessage, showAgents, messages, isLoadingNextStep]);

  const handleSendMessage = async (content: string) => {
    // Before adding the message, check if this is the initial response to the welcome message
    const isRespondingToWelcome = messages.length === 2 && 
    ...
    // Prevent multiple triggers of the same flow step
    if (isLoadingNextStep) return;
    
    // If responding to welcome message, proceed to next step
    if (isRespondingToWelcome) {
      proceedToNextDemoStep();
      return;
    }
    
    // For subsequent steps in the demo flow
    ...
    if (currentStep > 0 && currentStep < demoConversationSteps.length && 
      ...
      // Only add conclusion message if we haven't already
      if (!messages.some(m => m.content?.includes("The demo flow has concluded"))) {
        addMessage({
          role: 'assistant',
