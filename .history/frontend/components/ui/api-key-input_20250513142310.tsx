  ...
  return (
    <Card className="w-full shadow-md">
      <form onSubmit={handleSubmit}>
        <CardHeader>
          <CardTitle>API Key for {serviceName}</CardTitle>
          {description && <CardDescription>{description}</CardDescription>}
        </CardHeader>
        <CardContent className="space-y-3">
          <Label htmlFor={`api-key-${serviceName}`}>Enter API Key for {serviceName}:</Label>
          <Input 
          ...
          />
           <p className="text-xs text-muted-foreground">
            Your API key will be handled securely. (This is a demo UI)
          </p>
        </CardContent>
        <CardFooter>
