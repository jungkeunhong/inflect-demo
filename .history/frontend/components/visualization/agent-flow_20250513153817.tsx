...
export default function AgentFlow({ agents, activeAgent, onAgentClick }: AgentFlowProps) {
  ...
  // Check if we have a workflow with specific agent names (Root, Detect, Plan, Act)
  const hasOnboardingWorkflow = 
    agents.some(a => a.name.includes("Root")) &&
    agents.some(a => a.name.includes("Detect")) &&
    agents.some(a => a.name.includes("Plan")) &&
    agents.some(a => a.name.includes("Act"));
  ...
  if (hasOnboardingWorkflow) {
    ...
    const workflowAgents = [
      ...
      agents.find(a => a.name.includes("Detect")),
      agents.find(a => a.name.includes("Plan")),
      agents.find(a => a.name.includes("Act"))
        ...
        {workflowAgents.map((agent, index) => {
          ...
          return (
            <div key={agent.id} className="flex items-start flex-1">
              <div
                className={cn(
                  "w-full border rounded-lg p-3 cursor-pointer transition-all shadow-sm hover:shadow-md h-40",
              ...
              >
                ...
                {/* Description */}
                <div className="text-xs text-gray-500 mt-1 line-clamp-3 overflow-hidden">
                  {agent.description}
                </div>
              </div>
      ...
      {agents.map((agent, index) => {
        ...
        return (
          <div key={agent.id} className="flex items-center">
            <div
              className={cn(
                "border rounded-lg p-3 cursor-pointer transition-all shadow-sm hover:shadow-md h-28 w-48",
                activeAgent?.id === agent.id && "ring-2 ring-primary ring-offset-2",
                status.borderColor,
                "bg-white"
              )}
