...
interface SegmentType {
  ...
  pattern: string;
  users: string;
  badgeType: "increase" | "decrease" | "moderateDecrease" | "moderateIncrease" | "unchanged" | "normal";
  badgeValue: string;
  accentColor: string;
  bgColor: string;
  icon: React.ElementType;
        ...
        <CardContent>
          ...
          <div className="space-y-6 mb-6">
            {segments.map((segment, idx) => (
              <Card key={idx} className={`bg-white border border-${segment.accentColor}-100`}>
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <div className={`p-2 bg-${segment.bgColor} rounded-full`}>
                        <segment.icon className={`h-4 w-4 text-${segment.accentColor}-600`} />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {segment.userList.map((user, uIdx) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <div className={`w-2 h-2 rounded-full bg-${segment.accentColor}-${user.riskScore && user.riskScore > 80 ? '500' : '400'}`} />
